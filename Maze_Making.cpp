...
constexpr int MAZE_HEIGHT = 11;    // row，你可以把它改成你要的數字，但要是奇數
constexpr int MAZE_WIDTH = 15;    // column，你可以把它改成你要的數字，但要是奇數
constexpr int BEGIN_Y = 1;
constexpr int BEGIN_X = 0;
...
constexpr std::pair<int, int> GROUND_Direction[4]{ { 2, 0 }, { 0, 2 }, { -2, 0 }, { 0, -2 } };
...
int main() {
    ...
    Maze *M = new Maze();
    Maze_Making *mk = new Maze_Making();
    int __case;
    while ( true ) {
        M->reset();
        std::cout << "input the case,\ncase 1:random prim\ncase 2:recursion make\ncase 3:recursive_division\nyour input:> ";
        std::cin >> __case;
        switch ( __case ) {
            case 1:
                mk->random_prim_make_maze( 1 );
                break;
            case 2:
                mk->recursion_make_maze();
                break;
            case 3:
                M->reset_wall_around_maze();
                mk->recursive_division( 1, 1, MAZE_HEIGHT - 2, MAZE_WIDTH - 2 );
                break;
        }

        std::cout << MAZE_HEIGHT - 2 << " " << MAZE_WIDTH - 2 << '\n';
